...
window_gap=$(cat .config/bspwm/bspwmrc | awk '$3=="window_gap" {print $4}')
screen_width=$(xdpyinfo | grep dimensions: | awk '{print $2}' | awk -F'x' '{print $1}')

offset=$((screen_width - window_gap * 2))

n=$(cat ~/.config/bspwm/panel/config.ini | awk '$1=="width" {print $3}')
nxy=$(cat ~/.config/bspwm/panel/config.ini | awk '$1=="offset-x" {print $3}')
